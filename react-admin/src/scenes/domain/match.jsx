...
const Match = () => {
  ...
  const [matches, setMatches] = useState([]); // 팀 데이터를 저장할 상태 추가
  ...
  useEffect(() => {
    ...
    getMatchList().then(matchList => {
      console.log(matchList); // 가져온 클럽 리스트 데이터를 확인하기 위해 콘솔에 출력
      setMatchList(matchList); // 클럽 리스트 데이터를 state에 저장
      setMatches(matchList); // 팀 상태 업데이트
      setLoading(false);
    })
      .catch(error => {
  ...
  // 로딩 중일 때의 처리
  if (loading) {
    return <Typography>Match Loading...</Typography>; // 로딩 중일 경우 텍스트 표시
  }
  ...
  return (
    <Box m="20px"> {/* 외부 마진 20px */}
      <Header title="Match" subtitle="Managing the Match" /> {/* 제목과 부제목 표시 */}
      <Box
      ...
      >
        ...
        <DataGrid
          ...
          rows={matches} // 가져온 팀 데이터를 행으로 사용
          columns={columns} // 정의한 컬럼을 사용
          getRowId={(row) => row.matchId} // 각 행의 ID로 matchId 사용
        />
