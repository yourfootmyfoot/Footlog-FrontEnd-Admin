...
import { getGuestList } from './apis/guestAPI';
...
const Guest = () => {
  ...
  const [guests, setGuests] = useState([]); // 팀 데이터를 저장할 상태 추가
  ...
  // guest 데이터를 가져오는 useEffect 훅
  useEffect(() => {

    // 로그인 여부 확인 API 호출
    api.get('http://localhost:8080/api/auth/status', { withCredentials: true })
      .then(response => {
    ...
    // guest 리스트 불러오기
    getGuestList().then(guestList => {
      console.log(guestList); // 가져온 클럽 리스트 데이터를 확인하기 위해 콘솔에 출력
      setGuestList(guestList); // 클럽 리스트 데이터를 state에 저장
      setGuests(guestList); // 팀 상태 업데이트
      setLoading(false);
    })
      .catch(error => {
  ...
  // 로딩 중일 때의 처리
  if (loading) {
    return <Typography>Guest Loading...</Typography>; // 로딩 중일 경우 텍스트 표시
  }
  ...
  return (
    <Box m="20px"> {/* 외부 마진 20px */}
      <Header title="Guest" subtitle="Managing the Guest" /> {/* 제목과 부제목 표시 */}
      <Box
      ...
      >
        ...
        <DataGrid
          ...
          rows={guests} // 가져온 팀 데이터를 행으로 사용
          columns={columns} // 정의한 컬럼을 사용
          getRowId={(row) => row.guestId} // 각 행의 ID로 matchId 사용
        />
